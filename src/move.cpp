...
Move::Move(int from_x,
           ...
           int pawn_moved_two_squares_to_y)
    : from_x(from_x), from_y(from_y), to_x(to_x), to_y(to_y),
      captured_piece(nullptr), promotion_piece_type(PieceType::EMPTY),
      capture_is_en_passant(false), first_move_of_moving_piece(first_move),
      pawn_moved_two_squares(pawn_moved_two),
      pawn_moved_two_squares_to_x(pawn_moved_two_squares_to_x),
      pawn_moved_two_squares_to_y(pawn_moved_two_squares_to_y)
{
...
Move::Move(int from_x,
           ...
           bool is_en_passant)
    : from_x(from_x), from_y(from_y), to_x(to_x), to_y(to_y),
      captured_piece(captured_piece), promotion_piece_type(PieceType::EMPTY),
      capture_is_en_passant(is_en_passant),
...
Move::Move(
    int from_x, int from_y, int to_x, int to_y, PieceType promotion_piece_type)
    : from_x(from_x), from_y(from_y), to_x(to_x), to_y(to_y),
      captured_piece(nullptr), promotion_piece_type(promotion_piece_type),
      capture_is_en_passant(false), first_move_of_moving_piece(false),
      pawn_moved_two_squares(false), pawn_moved_two_squares_to_x(-1),
      pawn_moved_two_squares_to_y(-1)
{
}
